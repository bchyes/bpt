...
namespace sjtu {
    template<class Key, class T, int M = 16, class Compare= std::less<Key>, class Compare_T=std::less<T>>
    class bpt {
    public:
        ...
        bool Compare_all(const value_type &a, const value_type &b) {
            Compare cpy;
            Compare_T cpy_t;
            return (cpy(a.first, b.first) ||
                    (!cpy(a.first, b.first) && !cpy(b.first, a.first) && cpy_t(a.second, b.second)));
        }
        ...
        void insert(const value_type &value) {
            ...
            while (1) {
                ...
                for (i = 0; i < now.length - 1; i++)
                    if (Compare_all(value, now.value[i]))
                        break;
                ...
                if (tmp.is_leave) {
                    ...
                    for (i = 0; i < tmp.length; i++) {
                        if (!Compare_all(value, tmp.value[i]) && !Compare_all(tmp.value[i], value)) {
                            file.close();
                            throw int();
                        }
                        if (Compare_all(value, tmp.value[i])) {
                    ...
                    if (tmp.length == M) {
                        ...
                        while (node_back.father) {
                            ...
                            for (i = 0; i < node_back.length - 1; i++) {
                                if (Compare_all(new_tmp.value[0], node_back.value[i])) {
        ...
        void erase(const value_type &value) {
            ...
            for (int i = 0; i < now.length; i++)
                if (!Compare_all(value, now.value[i]) && !Compare_all(now.value[i], value)) {
                            ...
                            if (tmp.length > (M) / 2) {
                                ...
                                while (1) {
                                    for (int k = 0; k < tmp.length - 1; k++) {
                                        if (!Compare_all(tmp.value[k], v_up) &&
                                            !Compare_all(v_up, tmp.value[k])) {
                            ...
                            if (tmp.length > (M) / 2) {
                                ...
                                while (1) {
                                    for (int k = 0; k < now.length - 1; k++) {
                                        if (!Compare_all(now.value[k], up) &&
                                            !Compare_all(up, now.value[k])) {
                        ...
                        if (now.pre != -1 || now.next != -1) {
                            if (now.pre != -1) {
                                ...
                                if (now.father == tmp.father) {
                                    ...
                                    file.read(reinterpret_cast<char *>(&now), sizeof(node));
                                    for (int k = 0; k < now.length - 1; k++) {
                                        if (!Compare_all(v_up, now.value[k]) &&
                                            !Compare_all(now.value[k], v_up)) {
                                    ...
                                    while (now.father != -1) {
                                        if (now.length <= (M - 2) / 2) {
                                            ...
                                            for (k = 0; k < tmp.length - 1; k++) {
                                                if (Compare_all(now.value[0], tmp.value[k]))
                                                    break;
                                            }
                                            if (k != tmp.length - 1) {
...
sjtu::bpt<sjtu::string, int,30> tree("file");
